...
describe('pre deploy event registration hook interfaces', () => {
  ...
  test('no project error', async () => {
    const hook = require('../../src/hooks/pre-deploy-event-reg')
    expect(typeof hook).toBe('function')
    await expect(hook({ appConfig: {} })).rejects.toThrowError(new Error('No project found, skipping event registration in pre-deploy-hook hook'))
  })
  ...
  test('events sdk not initialised correctly error', async () => {
    ...
    await expect(hook({ appConfig: { project: mock.data.sampleProject, events: mock.data.sampleEvents } })).rejects.toThrowError(new Error('Events SDK could not be initialised correctly. Skipping event registration in pre-deploy-hook hook'))
  })
  ...
  test('error in create registration', async () => {
    ...
    expect(mockEventsSdkInstance.createRegistration).toHaveBeenCalledWith(CONSUMER_ID, PROJECT_ID, WORKSPACE_ID,
      mock.data.hookDecodedEventRegistration2
    )
  })
  ...
  test('successfully create registration with delivery type defined', async () => {
    ...
    events.registrations['Event Registration 2'].delivery_type = 'journal'
    mockEventsSdkInstance.createRegistration.mockReturnValue(mock.data.createWebhookRegistrationResponse)
  ...
  test('error in update registration', async () => {
    ...
    expect(mockEventsSdkInstance.updateRegistration).toHaveBeenCalledWith(CONSUMER_ID, PROJECT_ID, WORKSPACE_ID, 'registrationId2',
      mock.data.hookDecodedEventRegistration2
    )
  })
